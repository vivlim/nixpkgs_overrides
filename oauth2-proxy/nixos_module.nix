...
with lib;
let
  cfg = config.services.oauth2_proxy_mastodon;

  # oauth2_proxy_mastodon provides many options that are only relevant if you are using
  # a certain provider. This set maps from provider name to a function that
  # takes the configuration and returns a string that can be inserted into the
  # command-line to launch oauth2_proxy_mastodon.
  providerSpecificOptions = {
    ...
    mastodon = cfg: {
      mastodon-url = cfg.mastodon.mastodon-url;
    };
  };
...
in
{
  options.services.oauth2_proxy_mastodon = {
    enable = mkEnableOption (lib.mdDoc "oauth2_proxy_mastodon");
    ...
    cookie = {
      ...
      name = mkOption {
        type = types.str;
        default = "_oauth2_proxy_mastodon";
        description = lib.mdDoc ''
  ...
  config = mkIf cfg.enable {
    ...
    services.oauth2_proxy_mastodon = mkIf (cfg.keyFile != null) {
      clientID = mkDefault null;
      clientSecret = mkDefault null;
      cookie.secret = mkDefault null;
    };

    users.users.oauth2_proxy_mastodon = {
      description = "OAuth2 Proxy";
      isSystemUser = true;
      group = "oauth2_proxy_mastodon";
    };

    users.groups.oauth2_proxy_mastodon = {};

    systemd.services.oauth2_proxy_mastodon = {
      description = "OAuth2 Proxy";
      path = [ cfg.package ];
